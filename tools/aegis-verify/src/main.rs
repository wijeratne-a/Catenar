//! aegis-verify: Verify BLAKE3 chain integrity of aegis-trace-wal.jsonl files.
...
#[derive(Parser)]
#[command(name = "aegis-verify")]
#[command(about = "Verify BLAKE3 chain integrity of aegis trace WAL")]
struct Args {
    /// Path to aegis-trace-wal.jsonl
...
"#,
        ...
        let path = std::env::temp_dir().join("aegis-verify-test.jsonl");

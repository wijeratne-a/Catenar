//! Aegis Enterprise key providers: AWS KMS and HashiCorp Vault Transit.
...
use aegis_api::keys::KeyProvider;
...
impl AwsKmsProvider {
    ...
    pub(crate) async fn new_with_api(
        key_id: String,
        signing_algorithm: SigningAlgorithmSpec,
        api: Arc<dyn KmsApi>,
    ) -> Result<Self> {
...
/// Build enterprise key provider for aws_kms or vault. Returns None for local/env
/// so the caller can fall back to core's build_key_provider.
pub async fn build_key_provider_enterprise() -> Result<Option<Arc<dyn KeyProvider>>> {
    ...
    match provider.as_str() {
        "aws_kms" => {
            ...
            Ok(Some(Arc::new(AwsKmsProvider::new(key_id).await?)))
        }
        "vault" => {
            ...
            Ok(Some(Arc::new(VaultProvider::new(mount_path, key_name).await?)))
        }
        _ => Ok(None),
    }
}
...
mod tests {
    ...
    struct MockKmsApi {
        pub_key: Vec<u8>,
        sig: Vec<u8>,
        sign_calls: std::sync::Mutex<Vec<(String, Vec<u8>)>>,
    }
    ...
    async fn aws_kms_provider_signs_using_api() {
        let api: Arc<dyn KmsApi> = Arc::new(MockKmsApi {
            pub_key: vec![1, 2, 3],
            sig: vec![9, 8, 7],
            sign_calls: std::sync::Mutex::new(Vec::new()),
        });
